...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  // Grid lines use only uLinesColor - no mixing with scan color
  vec3 gridCol = uLinesColor;

  // Scan effect is separate - only visible where scan is active, not on grid lines
  float scanIntensity = combinedPulse + combinedAura * 0.25;
  vec3 scanCol = uScanColor * scanIntensity;

  // Final color: grid lines stay pure, scan adds glow in non-line areas
  color = mix(scanCol, gridCol, lineVis);

  color = clamp(color, 0.0, 1.0);

  // Alpha based on line visibility and scan pulse
  float alpha = clamp(max(lineVis, scanIntensity * 0.5), 0.0, 1.0);
